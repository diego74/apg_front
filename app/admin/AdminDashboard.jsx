...
export default function AdminDashboard({ admin }) {
  ...
  return <div className={styles.shell}>
    <aside className={styles.sidebar}><div className={styles.brand}>APG <small>ADMIN</small></div><nav><a className={styles.active}>Resumen</a><a href="#eventos">Campeonatos</a><a href="#proximamente">Jugadores <span>Pronto</span></a><a href="#proximamente">Equipos <span>Pronto</span></a></nav><button className={styles.signOut} onClick={signOut}>Cerrar sesión</button></aside>
    <main className={styles.main}><header className={styles.header}><div><p>ADMINISTRACIÓN</p><h1>Panel de control</h1><small>Hola, {admin.email}</small></div><button onClick={() => setShowForm(!showForm)}>{showForm ? 'Cancelar' : '+ Nuevo torneo'}</button></header>
      ...
      <p id="proximamente" className={styles.note}>La gestión detallada de jugadores y equipos está preparada en la base de datos para su próxima pantalla administrativa.</p>
    </main>
  </div>;
}
